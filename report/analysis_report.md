...
- 相位使用 TNS 发现日期作为零点；当前 TNS 缓存没有最大光日期，不能写成相对最大光相位。
- 吸收线中心不再用高斯拟合，统一采用平滑、局部归一化后的吸收最低点。
- pEW 和 FWHM 采用非参数定义，不再从高斯模型参数换算。
- 只有人工检查后可靠的谱线进入 `adopt`；有明显混线、缺红移或局部窗口不稳定的测量保留为 `check`。

2026-06-18 按最新算法重跑后的人工检查结论：没有发现需要通过 `LINE_PARAM_OVERRIDES` 固化的新窗口修正。明显不可靠的自动最低点已经在 QC 中保留为 `check`，主要是 FVX/JLM 的个别过窄 Ca II H&K 谷和 LMP 的所有缺红移候选线。仍需降权解读的是若干浅线或 systematics 敏感线，例如 KID 的 Fe II 5169、KIE 的 Ca II NIR、以及 Ia 早期 Ca II H&K 的 pEW；这些行可以支持速度量级或 line ID，但不要用来做强 pEW/FWHM 物理结论。
...
- `assets/data/spectra_summary.csv`：每条光谱的观测时间、距发现日相位、波长覆盖和文件来源。
...
`line_diagnostics_qc.csv` 中新增或需要保留的列包括 `rest_wave_choice`、`line_blend_note`、`velocity_sys_kms`、`pEW_sys_A`、`FWHM_sys_A` 和 `n_systematic_variants`。写结论时优先引用形式误差，同时用这些 systematics 列判断结果是否对平滑窗口或连续谱边缘选择敏感。
...
误差棒是形式统计误差：速度误差来自最低点定位不确定度，pEW 误差来自局部归一化残差传播，FWHM 误差来自半深度交点附近的采样级估计。新版 pipeline 还会输出 `velocity_sys_kms`、`pEW_sys_A` 和 `FWHM_sys_A`，用于记录改变平滑窗口和连续谱边缘比例后的经验散布。它们仍不是完整误差预算，尚未包含 TNS 红移误差、波长定标误差、线混合和大气/仪器响应。
...
- 本轮人工检查没有为 Fe II 5169 写入额外窗口覆盖；问题主要是线本身浅和噪声/systematics 敏感，不是单纯窗口参数错误。
...
- Ca II NIR 是 Ca II triplet 的 blend-proxy 测量，且本轮 pEW systematics/随机误差都较大；正式结论可以提速度量级，但不要单独讨论其 pEW 强度。
...
- 不要把表中形式误差或新版 systematics 列当成完整误差预算；红移、定标和线混合仍没有完全包含。
