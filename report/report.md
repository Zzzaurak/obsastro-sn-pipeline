...
目标类型、发现日期和红移优先来自 TNS public catalog。本项目不再手动测红移；对有 TNS 红移的目标，将观测波长按
...
- `assets/data/spectra_summary.csv`：每条 FITS 光谱的观测日期、相位、波长覆盖、仪器信息。
...
误差棒是形式统计误差。速度误差由最低点波长不确定度传播；最低点波长不确定度由局部残差噪声和像素采样共同估计。pEW 误差由局部归一化残差噪声在吸收积分区内传播。FWHM 误差是半深度交点附近的采样级估计。这些误差没有包含 TNS 红移误差、波长定标系统误差、连续谱窗口选择系统误差和线混合带来的系统误差。
...
黑体颜色温度只作为连续谱形状 proxy。SN2026KID、SN2026KIE 和 SN2026LMP 的蓝端形状/红移条件不够可靠，因此温度不作为主要科学结论。

![Blackbody color temperature](assets/figures/final5_blackbody_temperature.png)

宿主/环境窄线只按局部线指数和 S/N 粗测，不能替代严格流量定标后的金属丰度、消光或星形成率诊断。
