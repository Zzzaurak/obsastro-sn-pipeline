...
黑体颜色温度只作为连续谱形状。新版产物会用 `T_qc_flag` 将非 Ia 或缺红移目标降为 `check`。这张演化图统一使用 `phase_days` 作为横坐标；SN2026LMP 缺少距发现日相位，因此不画入该演化图，只保留在 CSV 和单谱拟合检查中。SN2026KID、SN2026KIE 和 SN2026LMP 的蓝端形状/红移条件不够可靠，因此温度不作为主要科学结论。
